use ftfrs::{Archive, Record, StringRef, ThreadRef, Result};
use std::env;
use std::fs::File;
...
fn read_trace(file_path: &str) -> Result<()> {
    ...
    if display_count > 0 {
        ...
        if archive.records.len() > display_count {
            println!("... and {} more records", archive.records.len() - display_count);
        }
    }
...
fn write_sample_trace(file_path: &str) -> Result<()> {
    ...
    archive.records.push(Record::create_initialization(1_000_000)); // 1M ticks per second
    ...
    println!("Successfully wrote {} records to {}", archive.records.len(), file_path);
    ...
    println!("\nYou can view this trace with: ./trace_tool read {}", file_path);
