...
use ftfrs::{Archive, Argument, Record, StringRef, ThreadRef};
...
pub fn create_mixed_archive(num_events: usize, interned_pct: usize) -> Archive {
    ...
    archive
        .records
        .push(Record::create_initialization(1_000_000));

    archive
        .records
        .push(Record::create_thread(1, 0x1234, 0x5678));

    let category_ref = 1;
    let name_ref = 2;
    archive
        .records
        .push(Record::create_string(category_ref, "category".to_string()));
    archive
        .records
        .push(Record::create_string(name_ref, "event_name".to_string()));
    ...
    for (i, name) in arg_names.iter().enumerate() {
        archive
            .records
            .push(Record::create_string(10 + i as u16, name.to_string()));
    }
    ...
    for i in 0..num_events {
        ...
        let event = match i % 4 {
            0 => Record::create_instant_event(timestamp, ThreadRef::Ref(1), category, name, args),
...
pub fn bench_string_handling(c: &mut Criterion) {
    ...
    for string_size in [8, 16, 32, 64, 128, 256].iter() {
        ...
        group.bench_function(BenchmarkId::new("string_reference", string_size), |b| {
            b.iter_batched(
                || {
